...
| `src/cli/commands/` | CLI commands: `auth`, `configure-subscription`, `configure-trait-gating`, `deploy`, `dry-run-gate`, `dry-run-predicate-gate`, `export`, `get-collections`, `get-trait-config`, `hash`, `init`, `inspect`, `pay`, `register`, `set-collection-tokens`, `set-collections`, `smoke`, `update-metadata`, `validate`, `verify` |
...
| `src/lib/client/` | Authenticated HTTP clients: SIWE auth, x402 payment, external signer, paid authenticated fetch |
